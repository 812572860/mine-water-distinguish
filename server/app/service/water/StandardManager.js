...
class StandardManager {
  constructor(db) {
    this.db = db || require('../../model').StandardSample
  }
  // 新增标准水样信息
      ...
      [
        ...
        (_, cb) => {
          sample
            ...
            .catch(err => {
              cb({ error: true, message: '标准水样信息新增失败', info: err })
            })
        }
      ],
      function(err, res) {
      ...
      [
          ...
          this.db
            ...
            .then(sample => {
              if (sample) {
                ...
                cb({ error: true, message: '该标准水样信息不存在' })
              }
            })
            .catch(err => {
              cb({ error: true, message: '标准水样信息查询失败', err })
            }),
        // validate验证
        (sample, cb) => {
          return sample
        ...
        (sample, cb) =>
          sample
            ...
            .catch(err => {
              cb({ error: true, message: '标准水样信息信息修改失败', err })
            })
      ],
      function(err, res) {
        callback(err, res)
  ...
  remove(args, callback) {
    ...
    if (!id) {
      error = { error: true, message: '删除标准水样信息不能为空！' }
          ...
          this.db
            ...
            .then(sample => {
              if (sample) {
                ...
                error = { error: true, message: '该标准水样信息信息不存在' }
                cb(error)
              }
            })
            .catch(err => {
