module.exports = (dbContext, dataTypes) => {
  const StandardSample = dbContext.define(
    'StandardSample',
    {
      id: {
  ...
  StandardSample.isDuplicate = (config, callback) => {
    const query = {
  ...
  StandardSample.combineSort = function(sorter = []) {
  ...
  StandardSample.combineQuery = function ({ keyword = '' }) {
    if (keyword) {
      return {
        $or: [
          {
  ...
  StandardSample.fetchBy = function(
    { pager = {}, sorter = [], where = {} },
    callback
  ) {
    let options = {
      where,
      sorter: StandardSample.combineSort(sorter)
    }
    ...
    return StandardSample.findAll(options)
