  ...
  <div :loading="loading">
    <div>
      <el-button
        ...
        @click="onCreate"
      ></el-button>
      <el-button type="primary" icon="el-icon-download" @click="onImport"
        >导入</el-button
      >
...
export default {
  ...
  methods: {
    ...
    onCreate () {
      this.dialogTitle = '添加待判水样数据'
      this.componentId = 'New'
      this.dialogVisible = true
    },
    onEdit (row) {
      this.dialogTitle = `修改${row.sampleNo}待判水样数据`
      this.componentId = 'New'
      this.selectInfo = { ...row }
      this.dialogVisible = true
    },
    onDelete (row) {
      this.$confirm('此操作将删除该待判水样数据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deletePendingSample({ ...row }).then(() => {
          this.onSearch()
          this.$message({
            type: 'success',
            message: '删除成功!'
    ...
    onDetail (row) {
      this.dialogTitle = `${row.sampleNo}待判水样详细信息`
      this.componentId = 'Details'
      this.selectInfo = { ...row }
      this.dialogVisible = true
    },
    onImport () {
      this.dialogTitle = '批量导入待判水样数据'
